...
var m = angular.module(speakerModuleId, [ 'GenericServices' ])
  .value('appName', 'Speaker Module');

m.resolveCurrentUser = function(UserService) {
...
m.config(['$routeProvider', function ($routeProvider) {
    ...
    var resolveCurrentUser = {
      currentUser: m.resolveCurrentUser
    };
  ...
  }]).controller(speakerCtrlPrefix + 'HomeCtrl',function ($scope) {
  }).controller(speakerCtrlPrefix + 'MyTalksCtrl',function ($scope, $location, TalksService) {
    $scope.model = {
  ...
  }).controller(speakerCtrlPrefix + 'SubmitTalkCtrl',function ($scope, $route, UserService, Tags, Talks, EventService) {
    $scope.model = {
      talk: {
        tags: [],
        speakers: [
  ...
  }).controller(speakerCtrlPrefix + 'TalkDetailsCtrl', function ($scope, $routeParams, TalksService) {
    $scope.model = {
      talk: TalksService.byId($routeParams.talkId)
    };
  }).controller(speakerCtrlPrefix + 'GlobalCtrl', function ($scope, $timeout, $q, UserService, EventService, EventBus) {
    $scope.global = {};
